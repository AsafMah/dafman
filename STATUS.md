...
**Active milestone:** **M1 Ã¢â‚¬â€ Make it solid** (see `plans/plan-roadmap.prompt.md`).
...
---
## Last completed
- `643493d` Ã¢â‚¬â€ `chore(test): ignore insta *.pending-snap artifacts`
- `7d26d5d` Ã¢â‚¬â€ `test(m1): testing baseline (vitest + cargo test + insta + CI)`
- `5e05456` Ã¢â‚¬â€ `feat(m1): swap to Supercharged SDK pin + add tracing observability baseline`
- `0f03190` Ã¢â‚¬â€ `docs(plans): add observability plan`
- `993aeef` Ã¢â‚¬â€ `chore: scaffold standard repo files`
## Next concrete step
...
## M1 Ã¢â‚¬â€ Make it solid (IN PROGRESS)
Definition of done lives in `plans/plan-roadmap.prompt.md`.
- [x] **SDK swap Ã¢â€ â€™ Supercharged** (git pin `c5c5757e935152c0850a822532036f3eea2ec78e`).
- [x] **Observability baseline** Ã¢â‚¬â€ `tracing` + `tracing-subscriber` + `tracing-appender`; daily JSON file appender under `app_log_dir()`; dev stderr layer; `EnvFilter` from `DAFMAN_LOG`; `#[tracing::instrument]` on all Tauri commands (secret args skipped).
- [x] **Testing baseline** Ã¢â‚¬â€ Vitest + `@vue/test-utils` + `happy-dom`; `cargo test` + `insta` + `tempfile`; first lib unit test (`logging::init`), first IPC contract snapshot (`SessionEventPayload`); CI runs both.
- [x] **Centralized scripts** in `package.json` (`npm run test`, `npm run lint`, `npm run check`).
- [x] **Backend module refactor** to the architecture plan layout (`app/{error,events,state}.rs`, `ipc/commands/{client,session}.rs`). `AppError` (`thiserror`) replaces `String` returns; `tests/ipc_contract.rs` now imports the real `SessionEventPayload`.
...
- [ ] **Typed IPC** Ã¢â‚¬â€ hand-mirror in `src/ipc/types.ts`; evaluate `tauri-specta` for M2.
...
- [ ] **Real permission UX** Ã¢â‚¬â€ `PermissionService` replaces `ApproveAllHandler`; modal prompt.
...
- [ ] **One Playwright smoke test** (first-run Ã¢â€ â€™ create client Ã¢â€ â€™ create session Ã¢â€ â€™ send Ã¢â€ â€™ see streaming reply).
## Tests at a glance
| Surface | Runner | Status |
|---|---|---|
| Frontend unit (`src/lib/__tests__/`) | Vitest + happy-dom | 8 tests passing |
| Backend lib (`src-tauri/src/*.rs`) | `cargo test --lib` | 1 test passing (module: `logging`) |
| Backend integration (`src-tauri/tests/`) | `cargo test` | 1 snapshot passing |
| E2E (Playwright) | _not yet wired_ | Ã¢â‚¬â€ |
...
- Don''t add ad-hoc CSS hex colors Ã¢â‚¬â€ use `var(--p-*)` PrimeVue tokens. Per-session accent is the only exception.
