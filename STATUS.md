...
**Active milestone:** **M1 â€” Make it solid** (see `plans/plan-roadmap.prompt.md`).
...
---
## Last completed
- `643493d` â€” `chore(test): ignore insta *.pending-snap artifacts`
- `7d26d5d` â€” `test(m1): testing baseline (vitest + cargo test + insta + CI)`
- `5e05456` â€” `feat(m1): swap to Supercharged SDK pin + add tracing observability baseline`
- `0f03190` â€” `docs(plans): add observability plan`
- `993aeef` â€” `chore: scaffold standard repo files`
## Next concrete step
...
## M1 â€” Make it solid (IN PROGRESS)
Definition of done lives in `plans/plan-roadmap.prompt.md`.
- [x] **SDK swap â†’ Supercharged** (git pin `c5c5757e935152c0850a822532036f3eea2ec78e`).
- [x] **Observability baseline** â€” `tracing` + `tracing-subscriber` + `tracing-appender`; daily JSON file appender under `app_log_dir()`; dev stderr layer; `EnvFilter` from `DAFMAN_LOG`; `#[tracing::instrument]` on all Tauri commands (secret args skipped).
- [x] **Testing baseline** â€” Vitest + `@vue/test-utils` + `happy-dom`; `cargo test` + `insta` + `tempfile`; first lib unit test (`logging::init`), first IPC contract snapshot (`SessionEventPayload`); CI runs both.
...
- [ ] **Typed IPC** â€” hand-mirror in `src/ipc/types.ts`; evaluate `tauri-specta` for M2.
...
- [ ] **Real permission UX** â€” `PermissionService` replaces `ApproveAllHandler`; modal prompt.
...
- [ ] **One Playwright smoke test** (first-run â†’ create client â†’ create session â†’ send â†’ see streaming reply).
## Tests at a glance
...
| E2E (Playwright) | _not yet wired_ | â€” |
...
- Don''t add ad-hoc CSS hex colors â€” use `var(--p-*)` PrimeVue tokens. Per-session accent is the only exception.
