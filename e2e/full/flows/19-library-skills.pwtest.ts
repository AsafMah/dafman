...
test('skills tab renders groups by source from the discover RPC', async ({ page }) => {
  ...
  await expect(
    library.locator('.skill-group-title', { hasText: 'personal-copilot' }),
  ).toBeVisible();
...
test('toggling a skill pushes the full disabled list via setGloballyDisabledSkills', async ({
  page,
}) => {
  await page.goto(`/?testBridge=${encodeURIComponent(harness.wsUrl)}&autosession=1`);
  await page.locator('.lex-composer-input').first().waitFor({ state: 'visible', timeout: 15_000 });
  ...
  // #28 added a second per-session ("This session") toggle to each row. The
  // "Default" (global) toggle — what this flow asserts — is first in DOM order.
  const summarizeRow = library.locator('.skill-row').filter({ hasText: 'summarize' }).first();
  await summarizeRow.locator('.p-toggleswitch').first().click();
