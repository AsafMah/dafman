...
/// `SubmitOnEnter` reads on each Enter keystroke.
///
/// Decision: mention-active while the menu is open AND it either has a
/// selectable result OR is still loading. A SETTLED empty result set
/// (`@nomatch`) is NOT active, so plain Enter SENDS the raw `@nomatch`
/// text — matching slash-command zero-match semantics. Treating
/// "loading" as active avoids the async race where Enter pressed before
/// the (sub-ms, cached) file search returns would otherwise send
/// `@query` instead of selecting the result that was about to appear.
