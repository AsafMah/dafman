...
describe('JsonValueView', () => {
  ...
  test('renders an object as key/value rows', () => {
    ...
    const keys = Array.from(container.querySelectorAll('.jv-key')).map((k) => k.textContent);
  ...
  test('renders an array with indices', () => {
    ...
    const indices = Array.from(container.querySelectorAll('.jv-index')).map((e) => e.textContent);
  ...
  test('recurses into nested objects', () => {
    ...
    const allKeys = Array.from(container.querySelectorAll('.jv-key')).map((k) => k.textContent);
