...
describe("getToolRenderer", () => {
  ...
  test("read_file infers result language from file extension", () => {
    ...
    expect(
      r({ args: { file_path: "src/main.ts" }, toolName: "read_file" })
        .resultLanguage,
    ).toBe("typescript");
    expect(
      r({ args: { file_path: "README.md" }, toolName: "read_file" })
        .resultLanguage,
    ).toBe("markdown");
    expect(
      r({ args: { file_path: "package.json" }, toolName: "read_file" })
        .resultLanguage,
    ).toBe("json");
    expect(
      r({ args: { file_path: "unknown.xyz" }, toolName: "read_file" })
        .resultLanguage,
    ).toBe("text");
  });
