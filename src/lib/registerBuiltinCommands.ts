...
export function registerBuiltinCommands(opts: RegisterOptions = {}): void {
  ...
  // ---------- Catch-all: auto-register any new boolean toggles ----------
  // Walks `settings.appearance` + `settings.notifications` + `settings.permissions`
  // and synthesizes `Toggle <humanized>` commands for every boolean key
  // not already explicitly registered above. Idempotent (replace-by-id).
  // Future schema fields surface automatically — no need to remember to
  // add a command when adding a setting.
  ...
  function registerBooleanCatchall(
    sectionLabel: string,
    section: Record<string, unknown>,
    setter: (key: string, value: boolean) => Promise<unknown> | unknown,
  ): void {
    for (const [key, value] of Object.entries(section)) {
      if (typeof value !== 'boolean') continue;
      if (explicitlyCovered.has(key)) continue;
      registry.register({
        id: `settings.${sectionLabel}.${key}.toggle`,
        label: `Toggle ${humanize(key)}`,
        group: 'Settings',
        icon: 'pi pi-cog',
        hint: sectionLabel,
        keywords: [key, sectionLabel, 'toggle'],
        run: async () => {
          const current = section[key] as boolean;
          await setter(key, !current);
        },
      });
    }
  }
  watch(
    ...
    (s) => {
      registerBooleanCatchall(
        'appearance',
        s.appearance as unknown as Record<string, unknown>,
        async (k, v) => {
          if (k === 'streaming') await settingsStore.setStreaming(v);
          else if (k === 'enableMermaid') await settingsStore.setEnableMermaid(v);
        },
      );
      registerBooleanCatchall(
        'notifications',
        ...
        async (k, v) => {
          if (k === 'turnEnd') await settingsStore.setNotifications({ turnEnd: v });
          else if (k === 'waitingForInput')
            await settingsStore.setNotifications({ waitingForInput: v });
        },
      );
      registerBooleanCatchall(
        'permissions',
        s.permissions as unknown as Record<string, unknown>,
        async (k, v) => {
          if (k === 'defaultApproveAll') await settingsStore.setDefaultApproveAll(v);
        },
      );
    },
    { immediate: true, deep: true },
