...
//
// In the fixed Enter-sends keybinding scheme plain Enter sends a message,
// so the HIGH handler must DEFER to an open menu — otherwise
// `/model<Enter>` and `@file<Enter>` would send a message instead of
// selecting the highlighted option. The HIGH handler reads this registry
// synchronously on each keystroke to decide whether to defer. (Shift+Enter
// is also routed through the HIGH handler while a menu is open, so a soft
// line break wins over the LOW handler's modifier-blind selection.)
//
