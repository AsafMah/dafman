...
const emitEvent = (payload: SessionEventPayload) => broadcast('sessionEvent', payload);
const emitPending = (payload: PendingRequestPayload) => broadcast('pendingRequest', payload);
...
const handlers: Record<string, (args: unknown) => Promise<unknown>> = {
  ...
  respondToRequest: rpcGuard(async (args) =>
    sessions.respondToRequest(args as Parameters<typeof sessions.respondToRequest>[0]),
  ),
  ...
  setSessionSkillEnabled: rpcGuard(async (args) => {
    const { sessionId, name, enabled } = args as {
      sessionId: string;
      name: string;
      enabled: boolean;
    };
...
const controlHandlers: Record<string, (args: unknown) => Promise<unknown>> = {
  ...
  '__test.recordAudit': async (args) => {
    const { entry } = args as {
      entry:
        | (Omit<import('./app/observability/audit').PermissionAuditEntry, 'ts' | 'kind'> & {
            kind: 'permission';
          })
        | (Omit<import('./app/observability/audit').UrlAuditEntry, 'ts' | 'kind'> & {
            kind: 'url';
          });
    };
