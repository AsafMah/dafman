...
- **Lexical-backed chat composer + message display.** Replaces the PrimeVue `InputText` composer and the plain-`<p>` assistant/reasoning body with two new components, `MessageComposer.vue` and `MessageContent.vue`, both backed by [Lexical](https://lexical.dev) via the `lexical-vue` Vue 3 binding (`lexical-vue@0.14.1` + `lexical@0.38.1` + matching `@lexical/*` packages, all version-pinned to avoid duplicate-version drift). The composer is plain-text today (history + auto-focus + Enter-to-send / Shift+Enter newline / IME-safe); the display side renders full markdown via `@lexical/markdown`'s `TRANSFORMERS` (headings, fenced code, lists, links, etc.). Streaming assistant deltas are coalesced via `requestAnimationFrame` so a burst of 5–30 deltas/sec triggers at most one Lexical reconcile per frame. New `src/lexical/{theme,plugins,nodes}.ts` keep Lexical wiring out of SFCs; global `src/lexical/lexical.css` styles the theme classes Lexical injects into the DOM (scoped CSS can't reach those nodes). Composer height auto-grows up to 60 % of the chat tile via a `ResizeObserver`-published `--tile-height` custom property.

### Fixed

- **Composer typing was broken under WebView2** with the full `RichTextPlugin` + markdown-shortcut stack. Reverted the composer to `PlainTextPlugin` + History + AutoFocus as the safe baseline; the display side still renders full markdown. Composer-side markdown shortcuts (auto-format `**bold**`, `# heading`, etc. as you type) are deferred — needs a real WebView2 devtools session to isolate which plugin in the stack interferes with the input pipeline.
