...
### Added (2026-06-11 projects — per-workspace config overlay, issue #264)

- **Projects: per-workspace session config that auto-applies (#264 Phase 1).** Save a session's configuration (agent, enabled MCP servers, disabled skills, run mode, model + reasoning effort, approve-all) as a **project** keyed to its working directory — via a new "Save as project" action in the session details panel. When you create a session in a directory that has a saved project, its defaults are applied automatically (toasting a summary + any servers/skills that no longer exist). Resolution tier is `global defaults → project overlay → per-session change`. Backed by a Bun `ProjectService` (`<userData>/projects.json`, atomic writes) + `listProjects`/`getProjectForPath`/`saveProject`/`deleteProject`/`applyProjectToSession`/`captureProjectFromSession` RPCs, delta-encoded so a project never force-disables a newly-added global server. Auto-apply resolves the session's **real** cwd (a new `getSessionMetadata` RPC), so it works for plain new sessions that land in the default workspace — not just explicit-workspace ones. Phase 1 scope: storage + apply/capture + create-time auto-apply + the capture action (the Library Projects tab is Phase 2).
...
### Fixed (2026-06-10 composer cleanup — issues #233, #237)
