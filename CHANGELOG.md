...
- **`bun run install:canary`** builds the `canary` channel and runs its Setup installer (`tools/install-channel.ts`), giving you a dafman you can keep open *alongside* `bun run dev` without tripping the single-instance guard (different build channel → separate WebView2 + JSON state). The in-place `electrobun build` output is a packaged Setup stub + sibling `.tar.zst`, not a runnable tree, so the helper runs the installer rather than the bare `bin/launcher`.
...
- **Drag-and-dropped (and pasted) text/code files are now actually read by the agent — even when they live outside the session's working directory.** The composer turns a dropped `File` into a base64 `blob` attachment, but investigation of the bundled host CLI (`node_modules/@github/copilot/app.js`, `Jio`/`fae`/`wIn`) showed it only inlines `blob` attachments whose mimeType is a recognized image or a native office/PDF document — **every other blob (a `.ts`/`.py`/`.md` file, whose webview `File.type` is empty so it ships as `application/octet-stream`, and `text/markdown` command-result pills) was silently DROPPED**, so the model "couldn't find the file". `SessionRegistry.send` now detects non-inlinable blobs and stages them to a real temp file, sending a `type:'file'` attachment instead; the host reads those from disk via its `<tagged_files>` flow, which works for paths outside the cwd. The same fix repairs the "attach command result" pill, which had the identical drop. Boundary tests in `src-bun/__tests__/sessions.test.ts` and `src-bun/__tests__/attachmentStaging.test.ts`.
...
||||||| parent of 46fa09d (feat(library): surface agent load failures)
=======
### Fixed (#81 — 2026-05-31)
...
>>>>>>> 46fa09d (feat(library): surface agent load failures)
### Fixed (#83 — 2026-05-30)
