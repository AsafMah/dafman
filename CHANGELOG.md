...
- **Per-session options popover.** Replaces the inline "Reasoning" select in the chat header with a gear button that opens a PrimeVue `Popover` exposing the full set of session-level knobs the SDK supports: agent **Run mode** (`interactive` / `plan` / `autopilot`), **Reasoning view** (default / hidden / compact / expanded), **Auto-approve permissions** toggle, **Session name** rename, **Compact history** action, and **Reset approvals** action. New backend RPCs `getSessionMode` / `setSessionMode` / `getSessionName` / `setSessionName` / `compactSessionHistory` / `setSessionApproveAll` / `resetSessionApprovals` wrap `session.rpc.{mode,name,history,permissions}.*`. The store reacts to `session.mode_changed` events so backend-initiated mode changes (e.g. `/plan` command) stay reflected in the UI.
