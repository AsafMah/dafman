...
plugin({
  ...
  setup(builder) {
    builder.onLoad({ filter: /\.vue(\?[^.]+)?$/ }, ({ path }) => {
      const real = path.includes('?') ? path.substring(0, path.indexOf('?')) : path;
      ...
      const parserPlugins: Array<'typescript'> = isTs ? ['typescript'] : [];
      ...
      let scriptCode = rewriteDefault(scriptBlock.content, '__sfc_main__', parserPlugins);
